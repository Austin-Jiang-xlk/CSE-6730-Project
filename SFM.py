...
class SFM:
    """
    A forward simulation of the traditional Social Force Model (SFM) for pedestrian dynamics.
    ...
    def __init__(self, params=None):
        ...
        default_params = {
            ...
            'tau': 0.3,
            'ped_ped_strength_repul': 18.8, 
            'ped_ped_strength_navig': 1.7, 
            'ped_des_speed': 1.44, 
            'destination_sigma': 1.2,
            'des_strength': 9.9, 
            'mass': 1,  # mass of pedestrian agent
            'ped_ped_range_repul': 0.8, 
            'ped_ped_range_navig': 0.8, 
            'anisotropy_lambda_repul': 0.8, 
            'anisotropy_lambda_navig': 0.9, 
            'ped_single_alpha': 0.5, 
            'ped_veh_strength': 33.5, 
            'ped_veh_decay': 0.4, 
            'anisotropy_lambda_p2v': 0.3
        }
    ...
    def simulate(self, ped_csv_path, veh_csv_path):
        ...
        for frame in range(start_frame, end_frame + 1):
            ...
            # Remove pedestrians that have reached their end frame (they leave the scene)
            ...
            # Get current vehicles (if any) at this frame and log them
            ...
            for pid, state in active_peds.items():
                ...
                # Compute destination desired velocity
                ego = state
